...
async fn main() -> Result<()> {
    ...
    bindgen!({world: "reactive-graph-plugin", path: "../reactive-graph-plugin-api/wit/reactive-graph-plugin.wit", async: true});
    ...
    let component = Component::from_file(&engine, "target/wasm32-wasip1/debug/reactive_graph_custom_plugin.wasm")?;
    let plugin = ReactiveGraphPlugin::instantiate_async(&mut store, &component, &linker).await?;

    // Here our `greet` function takes one name parameter,
    // but in the Wasmtime embedding API the first argument is always a `Store`.
    let greeting = plugin.call_greeting(&mut store, "Andreas").await?;
    println!("{greeting}");
    Ok(())
}
